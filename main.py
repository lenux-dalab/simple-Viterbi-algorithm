...
def viterbi_classical(transition_matrix, emission_probability_matrix, initial_probability_matrix, observations):
    ...
    """
    probabilities_of_most_probable_paths = np.zeros((observations.shape[0], initial_probability_matrix.shape[-1], ))
    most_probable_path_sequence = np.zeros((observations.shape[0], initial_probability_matrix.shape[-1], ))
    probable_path_probabilities = emission_probability_matrix[:, int(observations[0])] * initial_probability_matrix
    """
    V_1(x_1) = \psi(x_1)
    """
    ...
    for observation in range(1, len(observations)):  # Sequentially
        ...
        psi = emission_probability_matrix[:, int(observations[observation])] * transition_matrix
        old_probable_path_probabilities = probable_path_probabilities
        probable_path_probabilities = np.zeros(probable_path_probabilities.shape)
        ...
        for _psi in range(psi.shape[0]):
            probable_path_sequence[_psi] = np.argmax((psi[:, _psi] * old_probable_path_probabilities))
            probable_path_probabilities[_psi] = (psi[:, _psi] * old_probable_path_probabilities)[probable_path_sequence[_psi]]
    ...
    for observation in reversed(range(len(observations)-1)): # Sequentially
        # x^{*}_{k-1} = u_{k-1}(x^{*}_k)
        map_path[observation] = int(most_probable_path_sequence[observation+1][map_path[observation+1]])
    # print results
    print(map_path)
...
viterbi_classical(transition_probability_matrix, emission_probability_matrix, inital_probability_matrix, observations_matrix)
